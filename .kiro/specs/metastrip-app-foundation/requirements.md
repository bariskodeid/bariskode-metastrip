...
MetaStrip is a Flutter mobile application (Android 7.0+, iOS 13.0+) that enables users to view and remove metadata from various file types completely offline. The application provides comprehensive metadata extraction capabilities across 40+ file formats including images, videos, audio files, and documents, with four distinct removal modes to accommodate different privacy and technical preservation needs.
