...
- **Not current support:** APK/EPUB removal, TIFF/TIF removal, HEIF, RAW, M4V, legacy Office, GZ/BZ2. TIFF/TIF extraction remains available, but removal is disabled pending a future structural writer/POC. HEIC is allowlisted but has no format extractor. Video, recursive archive-member cleanup, and granular audio removal other than the WAV `LIST INFO` slice are deferred. WAV selective removal is limited to exact stable IDs for the eleven allowlisted INFO subchunk codes; AIFF selective removal remains disabled. Office selective removal is limited to exact stable IDs for standard core/app properties; ODF selective removal is limited to ten exact stable IDs for canonical `dc`/`meta` fields in the bounded root `meta.xml` part. Nested `meta.xml` names are outside that strict scope. Custom/user-defined ODF properties and legacy Office properties are unsupported. Device/SAF validation and stress testing remain pending; SAF output is attempted-unverified.
