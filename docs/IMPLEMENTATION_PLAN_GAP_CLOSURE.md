...
**Status:** In progress — Phases 0-1 complete, including the bounded ODF selective slice; Phase 2 BMP subset enabled, Phase 3 PDF structural cleanup remains pending, and Phase 4 ZIP-only container cleanup is implemented with device/SAF stress validation still pending
**Version:** 1.1
**Created:** 2026-08-08
...
FLAC Vorbis comment keys, WAV INFO fields, exact allowlisted Open XML core/app
properties, and the bounded ODT/ODS/ODP ODF property slice;
...
**Status:** Complete for PNG text, PDF Info, FLAC Vorbis comment-key, WAV INFO,
bounded ODT/ODS/ODP ODF property, and DOCX/XLSX/PPTX standard core/app property
slices (2026-08-09). Broader formats and modes remain follow-up work.

### Problem to solve

PNG, PDF, FLAC, WAV INFO, Open XML, and the bounded ODF slice now support their
documented stable field IDs end to end, while other formats reject selective
...
7. [x] Add WAV-only field-level removal. Stable IDs and descriptors cover the
...
Preserved-field reporting is not claimed in the current result model. For local
PNG and ODF output, the persisted clean copy is read back, reparsed, and
validated against the selected stable IDs and preservation invariants. SAF
generated bytes are validated before writing, but persisted SAF readback remains
unverified and scheduled for device testing. For PDF, requested IDs and the
best-effort warning are reported, but removed IDs and output validation are
...
- [x] Local persisted PNG and ODF output validation verifies selected fields are
  absent and the relevant preservation invariants hold.
...
- [x] Widget, BLoC, repository, datasource, and stripper coverage exercises the
  implemented selective behavior, including the bounded ODF slice.
- [ ] Preserved fields are identified and verified; no such claim is made by
  the current report.
- [x] Per-property selective removal is implemented for Office and ODF formats.
  Open XML is bounded to standard core/app parts; ODF is bounded to ten exact
  canonical-root properties. Both fail closed on malformed or ambiguous
  packages and report stable-ID removal/absence facts. Audio selective support
  is partial: FLAC and WAV INFO are implemented, while MP3, OGG, and Opus remain
  deferred.
