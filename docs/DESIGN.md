...
### 2.6 Custom Theme
User dapat memilih:
- Background color (HSL picker — hue, saturation, lightness)
- Accent color (HSL picker)
- Text color (auto-generated berdasarkan bg contrast, atau manual)
...
Body:
  Section 1: Queue (file list to process)
    Per file card (compact):
      [thumbnail 40×40] [filename] [removal mode selector dropdown]
      [filesize] · [metadata count] · [remove from queue icon]

  Section 2: Processing Options
    Mode selector: chip group (Full Strip | Selective | Anonymize | Preserve Technical)
    Apply to: All files / Selected files
...
Body (grouped sections):
  ...
  APPEARANCE:
    ...
    - [preview strip of 8 color dots]

  STORAGE:
    - Output Folder [path, tap to change]
    - Folder Structure [dropdown: flat/by date/by type]
    - Naming Template [text input atau preset selector]
    - Keep Original [toggle]

  PROCESSING:
    - JPEG Quality [slider 60–100, current value display]
    - Max Concurrent Files [segmented control: 1/2/4/8]
    - Auto-confirm [toggle]
    - Compute Hashes Automatically [toggle]

  APP MANAGEMENT:
    - Clear App Cache [right: "X.X MB"] → dengan loading + success snackbar
    - Reset App Data → 2-step confirmation dialog
    - Export Settings → file picker save
    - Import Settings → file picker open
  ...
  ABOUT:
    ...
    - Report a Bug [external link/email]
```
...
Body:
  ...
  Preset grid (2 columns):
    Per preset card:
      - 3 color swatches (bg, text, accent)
      - Theme name (IBM Plex Mono 12sp)
      - Selected indicator: border Accent Primary + checkmark
