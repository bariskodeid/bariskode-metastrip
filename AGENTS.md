...
Flutter app (Dart 3, package name `metastrip`). Reads metadata from an allowlisted set and strips metadata from the registered 18-extension Remover set. Offline-only.
...
- **Remover MVP scope**: the registry currently includes JPG/JPEG, PNG, PDF, MP3, FLAC, OGG, Opus, WAV, AIFF, DOCX, XLSX, PPTX, ODT, ODS, ODP, GIF, and WebP. PDF removal remains best-effort DocInfo only. Video, archives, HEIC/HEIF, RAW, granular audio removal, per-property Office removal, and the Full Strip/Selective/Anonymize/Preserve Technical UI modes are deferred or unwired.
