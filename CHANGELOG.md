...
- Enabled BMP removal for strict canonical 24/32-bit Windows
  `BITMAPINFOHEADER`, `BI_RGB`, positive dimensions, and `bfOffBits == 54`.
  The clean copy preserves header/pixel payload bytes, zeroes reserved fields,
  normalizes size fields, and discards trailing bytes.
- BMP support is intentionally narrow and is not comprehensive BMP
  sanitization. Unsupported variants fail closed.
...
- Added stable `MetadataFieldId` values and per-file `StripPolicy` propagation
  ...
  boundaries. Unsupported or mismatched selective requests fail closed per
  file and never fall back silently to full cleanup.
- Added metadata-value-free `StripReport` results and result-screen reporting for
  requested, removed, already-absent, and unsupported field IDs, warnings,
  output validation, and re-encoding status.
- PNG selective cleanup is verified for local persisted artifacts: selected
  tEXt/iTXt keyword fields are removed, the clean copy is written, read back,
  revalidated, and successful reports identify the removed stable IDs. SAF
  generated bytes are validated before writing, but persisted SAF readback is
  unverified and remains scheduled for device validation.
- PDF selective cleanup attempts only the selected DocInfo keys using the
  existing best-effort byte scanner. Reports and UI warnings explicitly mark
  the attempt as unverified: removed field IDs are not claimed and structural
  validity or comprehensive metadata removal is not asserted.
- Android/iOS picker and SAF persisted-output readback, plus ZIP memory stress testing, remain
  scheduled release-hardening work; see
  `docs/DEVICE_AND_STRESS_TEST_PLAN.md`.
...
- Stable-ID selective flow for PNG tEXt/iTXt (per keyword) and PDF DocInfo
  (per Info key, 9 keys incl. `Trapped`) is wired from Viewer selection through
  the per-file policy and result UI. PNG removal is verified after local
  persisted-output readback; SAF persisted output remains unverified pending
  readback support and device validation. PDF remains a best-effort
  attempted/unverified operation with an explicit warning.
  Anonymize, Preserve Technical, and selective support for other formats remain
  planned/unwired; unsupported requests fail closed without full-strip fallback.
...
- Remover UI copy updated to advertise the registered remover registry
